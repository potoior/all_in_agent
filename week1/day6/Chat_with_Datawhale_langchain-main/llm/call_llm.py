...
@Desc_EN:   Encapsulate native interfaces of various large models into one interface
'''

"""
大语言模型调用模块 - Large Language Model Call Module

这个模块提供了一个统一的接口来调用多种大语言模型API，包括：
- OpenAI GPT系列模型
- 百度文心一言系列模型  
- 讯飞星火系列模型
- 智谱AI ChatGLM系列模型

This module provides a unified interface to call multiple large language model APIs, including:
- OpenAI GPT series models
- Baidu Wenxin Yiyan series models
- iFlytek Spark series models
- Zhipu AI ChatGLM series models
"""
...
import websocket  # 使用websocket_client - Using websocket_client
...
def get_completion(prompt :str, model :str, temperature=0.1,api_key=None, secret_key=None, access_token=None, appid=None, api_secret=None, max_tokens=2048):
    """
    统一的大模型调用接口 - Unified Large Model Call Interface
    ...
    参数 / Parameters:
        ...
        secret_key: 百度文心模型需要的密钥 / Secret key for Baidu Wenxin model
        access_token: 访问令牌 / Access token
        appid: 讯飞星火模型需要的应用ID / App ID for iFlytek Spark model
        api_secret: 讯飞星火模型需要的密钥 / API secret for iFlytek Spark model
        max_tokens: 返回最长序列长度 / Maximum sequence length to return
        
    返回 / Returns:
        str: 模型生成的回复 / Model generated response
    """
    
    # 调用OpenAI GPT模型 - Call OpenAI GPT model
    if model in ["gpt-3.5-turbo", "gpt-3.5-turbo-16k-0613", "gpt-3.5-turbo-0613", "gpt-4", "gpt-4-32k"]:
        return get_completion_gpt(prompt, model, temperature, api_key, max_tokens)
    
    # 调用百度文心一言模型 - Call Baidu Wenxin Yiyan model
