"""
个人知识库助手 - Gradio 界面
Personal Knowledge Base Assistant - Gradio Interface

这个模块提供了一个基于Gradio的Web界面，用于与知识库进行交互。
支持多种大语言模型和嵌入模型，实现智能问答功能。

This module provides a Gradio-based web interface for interacting with the knowledge base.
Supports multiple large language models and embedding models for intelligent Q&A functionality.
"""

# 导入必要的库 - Import necessary libraries
import sys
import os                # 用于操作系统相关的操作，例如读取环境变量 / For OS operations like reading environment variables

# 添加父目录到系统路径，以便导入其他模块
# Add parent directory to system path for importing other modules
sys.path.append(os.path.dirname(os.path.dirname(__file__)))

import IPython.display   # 用于在 IPython 环境中显示数据，例如图片 / For displaying data in IPython environment
import io                # 用于处理流式数据（例如文件流）/ For handling streaming data
import gradio as gr      # Gradio库，用于创建Web界面 / Gradio library for creating web interfaces
...
# 加载环境变量配置 - Load environment variable configuration
# dotenv允许从.env文件中读取环境变量，避免硬编码敏感信息
# dotenv allows reading environment variables from .env files to avoid hardcoding sensitive information
_ = load_dotenv(find_dotenv())
# 支持的LLM模型配置 - Supported LLM model configuration
# 按平台分类的可用模型列表
LLM_MODEL_DICT = {
...
# 所有可用模型的扁平化列表 - Flattened list of all available models
LLM_MODEL_LIST = sum(list(LLM_MODEL_DICT.values()),[])
# ['gpt-3.5-turbo', 'gpt-3.5-turbo-16k-0613', 'gpt-3.5-turbo-0613', 'gpt-4', 'gpt-4-32k', 'ERNIE-Bot', 'ERNIE-Bot-4', 'ERNIE-Bot-turbo', 'Spark-1.5', 'Spark-2.0', 'chatglm_pro', 'chatglm_std', 'chatglm_lite']
# 默认配置 - Default configurations
INIT_LLM = "chatglm_std"                    # 默认LLM模型 / Default LLM model
EMBEDDING_MODEL_LIST = ['zhipuai', 'openai', 'm3e']  # 支持的嵌入模型 / Supported embedding models
INIT_EMBEDDING_MODEL = "m3e"                 # 默认嵌入模型 / Default embedding model
DEFAULT_DB_PATH = "./knowledge_db"          # 默认知识库路径 / Default knowledge base path
DEFAULT_PERSIST_PATH = "./vector_db/chroma" # 默认向量数据库路径 / Default vector database path

# 界面资源路径 - UI resource paths
...
def get_model_by_platform(platform):
    """
    根据平台名称获取对应的模型列表
    Get model list by platform name
    
    参数 / Parameters:
        platform (str): 平台名称 (openai, wenxin, xinhuo, zhipuai)
        
    返回 / Returns:
        list: 该平台支持的模型列表 / List of models supported by the platform
    """
...
class Model_center():
    """
    模型中心类 - 管理问答链的核心类
    Model Center Class - Core class for managing Q&A chains
    
    功能 / Features:
    - 存储和管理不同类型的问答链
    - Store and manage different types of Q&A chains
    - 支持带历史记录和不带历史记录的问答模式
    - Support both history-based and history-free Q&A modes
    
    属性 / Attributes:
        chat_qa_chain_self: 以 (model, embedding) 为键存储的带历史记录的问答链
        Dictionary storing Q&A chains with history, keyed by (model, embedding)
        qa_chain_self: 以 (model, embedding) 为键存储的不带历史记录的问答链
        Dictionary storing Q&A chains without history, keyed by (model, embedding)
    """
    def __init__(self):
        # 初始化问答链存储字典
        # Initialize Q&A chain storage dictionaries
        self.chat_qa_chain_self = {}  # 带历史记录的问答链 / Q&A chains with history
        self.qa_chain_self = {}       # 不带历史记录的问答链 / Q&A chains without history

    def chat_qa_chain_self_answer(self, question: str, chat_history: list = [], model: str = "openai", embedding: str = "openai",
     temperature: float = 0.0, top_k: int = 4, history_len: int = 3, file_path: str = DEFAULT_DB_PATH, persist_path: str = DEFAULT_PERSIST_PATH):
        """
        调用带历史记录的问答链进行回答
        Call Q&A chain with conversation history for answering
        
        参数 / Parameters:
            question: 用户问题 / User question
            chat_history: 聊天历史 / Chat history
            model: LLM模型名称 / LLM model name
            embedding: 嵌入模型名称 / Embedding model name
            temperature: 温度参数，控制生成随机性 / Temperature parameter controlling generation randomness
            top_k: 返回最相似的k个文档 / Return top k most similar documents
            history_len: 历史记录长度 / History length
            file_path: 知识库文件路径 / Knowledge base file path
            persist_path: 向量数据库持久化路径 / Vector database persistence path
            
        返回 / Returns:
            tuple: (错误信息, 更新后的聊天历史) / (Error message, Updated chat history)
        """
        # 检查问题是否有效 - Check if question is valid
        if question == None or len(question) < 1:
            return "", chat_history
        
        # 这里会不会导致不同用户使用同一个组合链，然后两个人的组合链的记忆出现混乱？
        # TODO 每个用户使用一个组合链，互不干扰
        try:
            # 如果指定模型组合的问答链不存在，则创建新的问答链
            # If Q&A chain for specified model combination doesn't exist, create a new one
            ...
            # 获取问答链并生成回答 - Get Q&A chain and generate answer
            chain = self.chat_qa_chain_self[(model, embedding)]
            # 返回空字符串（无错误）和更新后的聊天历史
            # Return empty string (no error) and updated chat history
    ...
    def qa_chain_self_answer(self, question: str, chat_history: list = [], model: str = "openai", embedding="openai", temperature: float = 0.0, top_k: int = 4, file_path: str = DEFAULT_DB_PATH, persist_path: str = DEFAULT_PERSIST_PATH):
        """
        调用不带历史记录的问答链进行回答
        Call Q&A chain without conversation history for answering
        ...
        try:
            ...
            # 将问题和回答添加到聊天历史 - Add question and answer to chat history
            chat_history.append(
...
def format_chat_prompt(message, chat_history):
    ...
    """
    最后就是
    第一轮：user ：xxx  Assistant：xxx
    第二轮：user ：xxx  Assistant：xxx
    第三轮：user ：xxx  Assistant：xxx
    """
